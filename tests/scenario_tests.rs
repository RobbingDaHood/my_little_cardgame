...
fn woodcutting_encounter_ids(client: &Client) -> Vec<usize> {
    let encounter_ids = encounter_hand_ids(client);
    encounter_ids.into_iter().filter(|&id| id == 23).collect()
}
...
fn scenario_woodcutting_encounter_full_loop() {
    ...
    let (status, _) = post_action(
        &client,
        r#"{"action_type":"EncounterPickEncounter","card_id":23}"#,
    );
    assert_eq!(status, Status::Created, "PickEncounter should succeed");
    ...
    assert_eq!(
        encounter
            .get("tree_tokens")
            .and_then(|v| v.get("TreeHealth"))
            .and_then(|v| v.as_i64()),
        Some(15),
        "Tree Health should start at 15"
    );
    ...
    loop {
        let mut round_turns = 0;
        while play_one_woodcutting_card(&client) {
            round_turns += 1;
            assert!(
                round_turns < 50,
                "Woodcutting round should end within 50 turns"
            );
        }
        total_encounters += 1;
        ...
        if last_outcome == "PlayerWon" {
            ...
            assert!(
                lumber > 0,
                "Player should have Lumber after winning woodcutting"
            );
        }
    ...
    assert!(
        total_encounters > 1,
        "With 100 durability, player should survive multiple woodcutting encounters"
    );
    ...
    if last_outcome == "PlayerLost" {
        ...
        assert_eq!(
            exhaustion, 0,
            "Woodcutting has no penalties; Exhaustion should be 0"
        );
        let final_durability = player_token(&client, "WoodcuttingDurability");
        assert_eq!(
            final_durability, 0,
            "Player woodcutting durability should be 0 when losing woodcutting"
        );
    }
    ...
    if last_outcome == "PlayerLost" || last_outcome == "PlayerWon" {
        ...
        if last_outcome == "PlayerLost" {
            assert_eq!(
                status,
                Status::Created,
                "Should be able to scout after woodcutting loss"
            );
        }
    }
}
