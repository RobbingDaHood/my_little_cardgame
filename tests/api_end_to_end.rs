...
fn hello_world() {
    ...
    // Verify Library cards are initialized (4 PlayerCardEffect + 4 EnemyCardEffect + Attack, Defence, Resource, CombatEncounter + 3 Mining + 1 MiningEncounter + 3 Herbalism + 1 HerbalismEncounter + 3 Woodcutting + 1 WoodcuttingEncounter)
    let library_cards = get_library_cards(&client);
    assert_eq!(24, library_cards.len());
