...
fn hello_world() {
    ...
    assert_eq!(108, library_cards.len());

    // Verify card counts: attack/defence have deck:15 hand:5, resource has deck:35 hand:5
    for card in &library_cards[9..11] {
        assert_eq!(card.counts.deck, 15);
        assert_eq!(card.counts.hand, 5);
        assert_eq!(card.counts.discard, 0);
    }
    assert_eq!(library_cards[11].counts.deck, 35);
    assert_eq!(library_cards[11].counts.hand, 5);

    // Verify card kinds
    assert_eq!(library_cards[9].kind["card_kind"], "Attack");
    assert_eq!(library_cards[10].kind["card_kind"], "Defence");
    assert_eq!(library_cards[11].kind["card_kind"], "Resource");
    assert_eq!(library_cards[12].kind["card_kind"], "Encounter");
