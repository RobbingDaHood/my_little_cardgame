use my_little_cardgame::deck::Deck;
...
fn test_play_defence_moves_card_to_discard() {
    ...
    let deck_before_json: serde_json::Value = serde_json::from_str(&resp_before.into_string().expect("body")).expect("json");
    ...
    let deck_after_json: serde_json::Value = serde_json::from_str(&resp_after.into_string().expect("body")).expect("json");
...
fn test_enemy_play_adds_dodge_to_enemy_in_defending() {
    ...
    let combat_json: serde_json::Value = serde_json::from_str(&resp_combat.into_string().expect("body")).expect("json");
    let enemy_tokens = &combat_json["enemies"][0]["tokens"];
    // Expect at least one token entry with token_type == "Dodge"
    let has_dodge = enemy_tokens.as_array().unwrap_or(&vec![]).iter().any(|t| t["token_type"] == "Dodge");
    assert!(has_dodge);
}
...
fn test_seed_determinism_for_enemy_selection() {
    ...
    let resp = client.post("/player/seed").header(Header { name: Uncased::from("Content-Type"), value: Cow::from("application/json") }).body(seed_body).dispatch();
    assert_eq!(resp.status(), Status::Ok);
    client.post("/combat/enemy_play").dispatch();
    let resp1 = client.get("/combat").dispatch();
    let combat1: serde_json::Value = serde_json::from_str(&resp1.into_string().expect("body")).expect("json");

    // Capture which unit card had Hand decreased by comparing first defence deck Hand count
    let hand1 = combat1["enemies"][0]["defence_deck"][0]["state"]["Hand"].as_u64().unwrap_or(0);
    ...
    let combat2: serde_json::Value = serde_json::from_str(&resp2.into_string().expect("body")).expect("json");
    let hand2 = combat2["enemies"][0]["defence_deck"][0]["state"]["Hand"].as_u64().unwrap_or(0);
...
fn test_advance_phase_rotates_state() {
    ...
    let combat_json: serde_json::Value = serde_json::from_str(&resp.into_string().expect("body")).expect("json");
    ...
    assert_eq!(resp.status(), Status::Created);
    let resp2 = client.get("/combat").dispatch();
    let combat_json2: serde_json::Value = serde_json::from_str(&resp2.into_string().expect("body")).expect("json");
    assert_eq!(combat_json2["state"], "Attacking");

    // advance -> Resourcing
    client.post("/combat/advance").dispatch();
    let resp3 = client.get("/combat").dispatch();
    let combat_json3: serde_json::Value = serde_json::from_str(&resp3.into_string().expect("body")).expect("json");
    assert_eq!(combat_json3["state"], "Resourcing");

    // advance -> Defending
    client.post("/combat/advance").dispatch();
    let resp4 = client.get("/combat").dispatch();
    let combat_json4: serde_json::Value = serde_json::from_str(&resp4.into_string().expect("body")).expect("json");
...
fn test_enemy_unit_hand_to_discard_on_play() {
    ...
    let combat_before: serde_json::Value = serde_json::from_str(&resp_before.into_string().expect("body")).expect("json");
    let hand_before = combat_before["enemies"][0]["defence_deck"][0]["state"]["Hand"].as_u64().unwrap_or(0) as u32;
    let discard_before = combat_before["enemies"][0]["defence_deck"][0]["state"]["Discard"].as_u64().unwrap_or(0) as u32;
    ...
    let combat_after: serde_json::Value = serde_json::from_str(&resp_after.into_string().expect("body")).expect("json");
    let hand_after = combat_after["enemies"][0]["defence_deck"][0]["state"]["Hand"].as_u64().unwrap_or(0) as u32;
    let discard_after = combat_after["enemies"][0]["defence_deck"][0]["state"]["Discard"].as_u64().unwrap_or(0) as u32;
...
fn test_dodge_consumed_by_enemy_attack() {
    ...
    let action_json = r#"{ "PlayCard": 1 }"#;
    let resp = client.post("/action").header(Header { name: Uncased::from("Content-Type"), value: Cow::from("application/json") }).body(action_json).dispatch();
    assert_eq!(resp.status(), Status::Created);

    // Verify player has dodge token
    let token_resp = client.get("/player/tokens").dispatch();
    assert_eq!(token_resp.status(), Status::Ok);
    let tokens_before: serde_json::Value = serde_json::from_str(&token_resp.into_string().expect("body")).expect("json");
    let dodge_before = tokens_before.as_array().unwrap_or(&vec![]).iter().find(|t| t["token_type"]=="Dodge").and_then(|t| t["count"].as_u64()).unwrap_or(0);
    let health_before = tokens_before.as_array().unwrap_or(&vec![]).iter().find(|t| t["token_type"]=="Health").and_then(|t| t["count"].as_u64()).unwrap_or(0);
    ...
    let tokens_after: serde_json::Value = serde_json::from_str(&token_resp2.into_string().expect("body")).expect("json");
...
fn test_player_kills_enemy_and_combat_ends() {
    ...
    let resp = client.post("/cards").header(Header { name: Uncased::from("Content-Type"), value: Cow::from("application/json") }).body(card_json).dispatch();
    assert_eq!(resp.status(), Status::Created);
    let location = resp.headers().get_one("location").expect("location header").to_string();
    let card_id: usize = location.trim_start_matches("/cards/").parse().expect("Invalid card id");
    ...
    let resp = client.post(format!("/decks/0/cards")).header(Header { name: Uncased::from("Content-Type"), value: Cow::from("application/json") }).body(deck_card_json).dispatch();
    assert_eq!(resp.status(), Status::Created);
    ...
    assert_eq!(resp.status(), Status::Ok);
    let result_json: serde_json::Value = serde_json::from_str(&resp.into_string().expect("body")).expect("json");
