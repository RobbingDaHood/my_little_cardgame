...
- Health initialization: player Health is set to 2000 when picking an encounter only if current Health is 0. Health persists across encounters within a game; it is not reset between encounters.
...
- Rest (recovery encounter): a pacing/recovery encounter type. Rest cards are encounter-internal (like OreCard/FishCard/PlantCard), not player library cards. 3 card templates (2 Stamina, 2 Health, 1 Mixed) are rolled into 5 concrete cards at game init, each with 5 copies (25 total in the rest deck). At encounter start, 5 cards are drawn. The player picks 1 card; its recovery effect (stamina and/or health with caps) takes effect immediately and the encounter is always won. Most rest cards cost Fish and Plant (herbs) tokens; the mixed card is cost-free but gives less total recovery. Rest encounters compose ~20% of the encounter deck (hand: 4). Defined in `src/library/disciplines/rest.rs`.
...
5. **Single-choice recovery** (Rest): draw 5 rest cards from the rest deck, pick 1 card. Recovery (stamina/health) is applied immediately and the encounter is always won. Costs are paid in Fish and Plant (herbs) tokens. No loss condition.
...
- **No enemy deck** (Woodcutting, Rest): no enemy or node deck at all. The encounter is purely about the player's card choices. Challenge comes from hand management and pattern construction (Woodcutting) or resource cost management (Rest).
